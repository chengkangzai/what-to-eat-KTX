 ...
 *
 * Taken from https://gist.github.com/AllanHasegawa/d1e251744275d16af45484b0dbac37ee
 *
...
object FirebaseIDGenerator {
    ...
    fun generateNextId(previousState: State, instant: Long = DEFAULT_INSTANT()): Result {
        ...
        val randCharsAsString =
            randChars.fold(StringBuilder(12)) { str, i -> str.append(PUSH_CHARS[i]) }
...
class FirebaseHelper {
    fun getTimestamp(): HashMap<String, Any> {
        return hashMapOf<String, Any>(
            "seconds" to Timestamp.now().seconds,
            "nanoseconds" to Timestamp.now().nanoseconds
        )
    }
}
